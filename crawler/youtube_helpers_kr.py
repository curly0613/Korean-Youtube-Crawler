...
everything_cool = re.compile(r"^[가-힣0-9\,\.\-\?\"\'\’\!\“\s\;\:\“\”\–\‘\’\’\/\\]+$")
leave_chars = re.compile(r"[^가-힣\s]")
# numbers are ignored
...
def get_all_subtitles(dir):
    ...
    for filename in Path(dir).walkfiles("*.ko.vtt"):
...
def get_video_file(subtitle_file):
    naive_video_file = subtitle_file.replace(".ko.vtt", ".mp4")
    webm_video_file = subtitle_file.replace(".ko.vtt", ".webm")
    if os.path.exists(naive_video_file) or os.path.exists(webm_video_file):
        return naive_video_file
    else:
        dumb_youtube_file = subtitle_file.replace(".ko.vtt", "")
...
def int_to_kr(num):
    ...
    if (num < 1000):
        if num % 100 == 0:
            ...
            if num // 100 == 1 :
                return '백 ' + int_to_kr(num % 100)
            return d[num // 100] + '백 ' + int_to_kr(num % 100)
    ...
    if (num < 10000):
        if num % 1000 == 0:
            ...
            if num // 1000 == 1 :
                return '천 ' + int_to_kr(num % 1000)
            return d[num // 1000] + '천 ' + int_to_kr(num % 1000)

    if (num < k):
        if num % 10000 == 0:
            if num // 10000 == 1 :
                return '만'
            return int_to_kr(num // 10000) + '만'
        else:
            if num // 10000 == 1 :
                return '만 ' + int_to_kr(num % 10000)
            return int_to_kr(num // 10000) + '만 ' + int_to_kr(num % 10000)


def normalize_numbers(input_str):
    input_str = input_str.replace('%', " 퍼센트")
    ...
    for number in numbers:
        if len(number) <= 10:
            number_str = int_to_kr(int(number))
...
def normalize_subtitle(input_str):
    ...
    # 콤마나 점 같은 경우 앞뒤에 숫자가 있으면 예외처리
    for i in [pos for pos, char in enumerate(input_str) if char == ','] :
        if input_str[i-1].isdigit() and input_str[i+1].isdigit() :
            input_str = input_str.replace(',', '')
    for i in [pos for pos, char in enumerate(input_str) if char == '.'] :
        if input_str[i-1].isdigit() and input_str[i+1].isdigit() :
            count = i+1
            tmp_txt = '점'
            while input_str[count].isdigit() :
                tmp_txt += int_to_kr(int(input_str[count]))
                count += 1
            input_str = input_str.replace(input_str[i:count], tmp_txt)
            with open("number.txt", "a") as file :
                file.write(input_str+"\n")
    input_str = input_str.replace(',', ' ').replace('.', ' ')
    input_str = re.sub(r"([가-힣0-9])\-([가-힣0-9])", r"\1\2", input_str, 0, re.IGNORECASE)
    input_str = input_str.replace("- ", " ")
    input_str = input_str.replace("— ", " ")
    input_str = input_str.replace('’', '\'').replace('‘', '\'').replace('ʻ', '\'').replace('´', '\'').replace('\'', " ").replace("&nbsp;", ' ')
    # substitute speaker info
    input_str = re.sub('<[^<]+?>', ' ', input_str)
    input_str = re.sub(r"[가-힣a-zA-Z0-9\s]+\:", " ", input_str)
...
def _get_transcript_google_web_asr(t, v):
    import tempfile
    try:
        with tempfile.NamedTemporaryFile(suffix=".wav") as f:
            extract_audio_part_segment(v, t["ts_start"], t["ts_end"], f.name)
            r = sr.Recognizer()
...
def google_speech_test(timings, threshold=0.65, samples=2, min_duration=2.5):
    ...
    print([(t["original_phrase"].lower(), s.lower()) for (t, s) in transcripts])
