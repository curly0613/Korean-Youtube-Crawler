import re
from youtube_helpers_kr import remove_overlapping_subtitles, \
...
class OverlappingSubtitlesRemover(BaseFilter):
    ...
    def __call__(self, input):
        ...
        print("remove overlap :", len(input['subtitles']))
        return input
...
class SubtitleMerger(BaseFilter):
    ...
    def __call__(self, input):
        ...
        print("sub merge :", len(input['subtitles']))
        return input
...
class SubtitleCaptionTextFilter(BaseFilter):
    ...
    def __call__(self, input):
        ...
        input['subtitles'] = list(filter(lambda s: all(s['original_phrase'].find(c) == -1
                                                  ...
                                                  subtitles))
        print("sub cap text filter :", len(input['subtitles']))
        return input
...
class CaptionRegexMatcher(BaseFilter):
    ...
    def __call__(self, input):
        ...
        print("cap regex match :", len(input['subtitles']))
        return input
...
class CaptionNormalizer(BaseFilter):
    def __call__(self, input):
        ...
        print("cap normalize :", len(input['subtitles']))
        return input
...
class CaptionLengthFilter(BaseFilter):
    ...
    def __call__(self, input):
        ...
        print("cap len filter :", len(input['subtitles']))
        return input
...
class CaptionDurationFilter(BaseFilter):
    ...
    def __call__(self, input):
        ...
        print("captionDurationFilter :", len(input['subtitles']))
        return input
...
class CaptionLeaveOnlyAlphaNumCharacters(BaseFilter):
    def __call__(self, input):
        ...
        print("captionleaveonlyAlphabetCharacter :", len(input['subtitles']))
        return input

class GoogleRandomSubsetWERFilter(BaseFilter):
    def __init__(self, num_samples_to_test=5, mean_wer_threshold = 0.7):
    ...
    def __call__(self, input):
        ...
        transcripts = [(s, _get_transcript_google_web_asr(s, input["video_file"])) for s in subset]
        ...
        print("google filter : ", len(input['subtitles']))
        return input

if __name__ == "__main__":
    from crawler.youtube_helpers import load_all_subtitles
    subtitles = load_all_subtitles("./../video/2oNoBDMGGioHow_to_Make_a_Picnic_Table_-_Plans_and_Instructions.kr.vtt")
    print(len(subtitles))
    input = {
        'subtitles' : subtitles,
        'video_file' : ''
    }
    good_chars_regexp = re.compile(r"^[a-zA-Z가-힣0-9\,\.\-\?\"\'\’\!\“\s\;\:\“\”\–\‘\’\’\/\\]+$")
    pipeline = Pipeline([
